...
const std::string BasicBee::BASE_ABBREV("Bee");

// Default constructor (location is default-null)
BasicBee::BasicBee(Space *location)
    : Bee(BASE_NAME, BASE_ABBREV, BASE_MAX_ARMOR, BASE_MAX_ARMOR, location, BASE_TARGETABILITY, BASE_MOVE_SIZE, BASE_NEGATIVE_MOVE_DIRECTION, BASE_ACTION_PHASE) {
...
void BasicBee::act() {
    ...
    if (target) {       // if target != null, we have a target
        if (attack(target))
            std::cout << "Bee inflicts " << attackPower << " damage on " << target->getName() << (target->isAlive() ? ".\n" : ", killing it.\n");
    }
}
